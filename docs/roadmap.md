...
- [ ] **Fase 5.0D.4A — Arquitetura do Editor Visual.** Gate exclusivamente arquitetural para definir um modelo seguro e sem perdas de edição visual de Markdown/markup Note-it: mapeamento bidirecional fonte↔visual; semântica de cursor, seleção e mutações; preservação integral de sintaxe desconhecida ou malformada; e interação entre os modos Visual e Markdown fonte. A persistência canônica do Core permanece inalterada, salvo decisão arquitetural separadamente aprovada que prove outra necessidade. **Nenhuma implementação de produção do editor visual é autorizada antes da aprovação desta arquitetura.**
- [ ] **Fase 5.0D.4B — Implementação do Editor Visual.** Autorizável somente após a aprovação da 5.0D.4A. Implementará o modo de edição visual, ocultando na experiência normal a sintaxe Markdown/HTML suportada sem alterar a fonte canônica; oferecerá alternância explícita para o modo Markdown; mapeará cursor, seleção e mutações com segurança; e preservará undo/redo, conflitos de revision, sinais e recuperação. **Não iniciada.**
- [ ] **Fase 5.0D.5 — Paridade Semântica: Matemática e Flashcards.** Auditar e implementar na TUI a semântica matemática do editor gráfico e a extração, apresentação e preservação segura de flashcards. Reutilizar ou compartilhar a semântica existente sempre que possível; implementações ingênuas e divergentes baseadas em regex ou `split` ficam proibidas. **Não iniciada.**
