...
- `contract.rs`: a superfície publicada — entradas tipadas, saídas estruturadas, códigos de erro e a lista de tools — escrita à mão em vez de derivada dos tipos do Core, para que uma renomeação lá não mude o schema em silêncio. O erro de compilação de uma renomeação aparece em `domain.rs`, que é o arquivo que importa os tipos do Core e os traduz; `contract.rs` não importa nada de `noteit-core`. É aqui que mora a regra central da fase: toda entrada que nomeia uma nota existente carrega `expected_revision: RevisionArgument`, nunca um `Option`.
