...
- **Apresentação ao executar sem argumentos:** `noteit` sozinho mostra o logotipo `NOTE-IT` em blocos, a versão vinda da própria versão do pacote, uma linha sobre o que o Note-it é e cinco comandos por onde começar — e encerra com código `0`. Não é um prompt, não espera entrada e não deixa nada para trás: nenhuma nota, janela, socket, lock ou store, nem sequer um carimbo de tempo alterado. O logotipo aparece só aí; `noteit ajuda`, os erros e o `--json` não o repetem.
- **Identidade visual discreta:** amarelo para a marca, magenta para o acento, e nenhuma terceira cor. Cores ANSI básicas, sem exigir true color. Cor nunca é a única forma de dizer alguma coisa — retirando todo o estilo, o texto continua o mesmo.
- **Apresentação consciente do terminal:** a largura vem do próprio terminal (`TIOCGWINSZ`), com `COLUMNS` apenas como reserva e só quando o valor é plausível; sem terminal, 80 colunas por suposição conservadora. Logotipo em blocos a partir de 54 colunas; entre 27 e 53, `NOTE-IT` escrito com os mesmos cinco comandos; abaixo de 27, `NOTE-IT`, versão e os dois comandos essenciais. Nenhuma largura produz linha maior que a janela.
- **Política de ANSI por canal:** cor só aparece quando o canal correspondente é um terminal compatível — a saída padrão ser um terminal não estiliza uma saída de erro redirecionada, nem o contrário. `NO_COLOR`, mesmo definido como string vazia, e `TERM=dumb` desligam a cor; `TERM=dumb` também dispensa a arte em blocos. Cano e redirecionamento recebem texto puro, determinístico e idêntico a cada execução.
