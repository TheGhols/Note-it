...
use noteit_core::control::{
    read_frame, write_frame, ControlRequest, ControlResponse, ControlResult, MAX_FRAME_BYTES,
};
...
impl Sandbox {
    ...
    fn run_with_stdin(&self, args: &[&str], stdin: &str) -> (i32, String, String) {
        let mut child = self
            .command(args)
            .stdin(Stdio::piped())
            .stdout(Stdio::piped())
...
impl FakeAuthority {
    fn start(sandbox: &Sandbox, behaviour: AuthorityBehaviour) -> Self {
