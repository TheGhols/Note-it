...
- **Fase 5.0D.5 — Paridade Semântica: Matemática e Flashcards.** A semântica do editor gráfico foi portada para a TUI, não reinterpretada, e a paridade é provada por fixtures cruzadas.
  ...
  - **Flashcards lendo a projeção (`noteit-tui/src/flashcards.rs`):** O editor gráfico extrai do documento ProseMirror e nunca do Markdown, porque uma expressão regular não enxerga que `::` está dentro de um fence, de um código inline ou de um destino de link. A TUI pergunta à projeção lossless da 5.0D.4B, que tem o mesmo conhecimento. Um cartão continua sendo projeção: nenhum arquivo paralelo, nenhum identificador escondido em comentário, nenhum byte escrito na nota.
  - **Prova cruzada:** Duas fixtures — `tests/fixtures/math-conformance.json` (52 linhas e 9 notas) e `tests/fixtures/flashcard-conformance.json` (29 notas) — geradas da implementação canônica e afirmadas pelos **dois** lados em suas próprias suítes. Comparar as implementações entre si provaria que concordam, não sobre o quê.
  - **A única divergência encontrada:** acima de 1e21 o canônico escreve `1e+40` e o Rust escrevia os quarenta e um dígitos. As duas grafias estão "certas" e só uma é a que aparece na outra janela — exatamente o tipo de diferença que uma reimplementação parecida deixaria passar.

- **Fase 5.0D.4B — Implementação do Editor Visual.** Os catorze portões da sequência obrigatória passaram na ordem, cada um com gate verde antes do seguinte.
  - **A sequência:** B.1 projeção lossless, P0 baseline, B.2 source map somente leitura, P1 gate pré-interativo, B.3 edição mínima, B.4 headings e fronteiras de bloco, P2 gate pré-inline, B.5 capacidades inline uma a uma, P3 gate pré-HTML, B.6 HTML canônico, P4 gate pré-blocos, B.7 blocos estruturados, B.P fechamento de performance, B.R fechamento adversarial.
  - **O que o leitor vê:** `Alt+V` alterna entre Markdown e Visual e a nota sempre abre em Markdown. No Visual, negrito parece negrito, cor é cor, marca-texto é fundo, tarefas mostram `☐`/`☑`, listas mostram `•` e links mostram só o rótulo. Tudo o que os gates ainda não sabem editar — fence, comentário, HTML desconhecido — continua **visível como fonte**, porque esconder o que não se pode editar diria ao leitor uma mentira sobre o que ele pode fazer.
  - **Recusa nunca é silêncio:** cada recusa nomeia a causa e aponta o caminho de volta ao Markdown, que continua sendo o fallback integral e permanente. A exceção é Backspace no início do documento, que é nada a fazer e não merece aviso.
  - **Garantias mantidas:** `Draft` continua sendo a única fonte mutável, toda mutação passa por `authority::perform_at` com a revision lida, conflito não sobrescreve, sinais preservam o rascunho e o terminal volta exatamente como estava — provado em pseudoterminal real.
  - **Cinco quadráticas encontradas por gates que rodam de verdade:** a maior fazia um `VisualDocument` de 200 KB levar 598 ms, e uma delas travou a suíte em vez de apenas atrasá-la. Nenhuma era visível lendo o código. Depois de células e slots construídos sob demanda por bloco, uma tecla numa nota de 64 KB caiu de 31,7 ms para 2,5 ms.
  - **Provas:** a suíte da TUI foi de 165 para 426 testes, incluindo property tests determinísticos, tabelas de mutação, Unicode, conteúdo adversarial, `TestBackend`, PTY real, conflito, recovery, sinais, redimensionamento e terminal estreito.

- **Fase 5.0D.4A — Arquitetura do Editor Visual.** Gate exclusivamente arquitetural, fechado após quatro rodadas e três revisões adversariais independentes. Nenhum código de produção foi escrito nesta fase e o `Cargo.lock` permaneceu byte-idêntico.
  - **Processo:** A primeira revisão independente bloqueou o desenho com 3 BLOCKERs e 21 MAJORs, cada um com contraexemplo concreto; a R3 os corrigiu; a segunda revisão confirmou dois blockers fechados, manteve um aberto e apontou oito defeitos introduzidos pela própria correção; a R4 os fechou; a terceira revisão aprovou com dois ajustes de uma frase. Nenhuma rodada emitiu o próprio veredito, e a R3 teve sua linha de auto-aprovação removida por isso.
  - **Contratos fixados:** `Lexeme` (partição física byte-exact), `Node` (árvore semântica) e `ProjectionRun` (saída visual derivada) como camadas distintas; `Generation` estritamente monotônica por sessão, incrementando em toda mutação do `Draft` inclusive reseat, undo e redo; precedência de fenced code, code span balanceado e escape **antes** de qualquer candidato HTML; algoritmo normativo de limite HTML quote-aware, document-bounded, com proteção até EOF para todo open tag sem close comprovado; escopo de bloco obrigatório na álgebra de seleção; três eixos ortogonais de classificação (reconhecimento, visibilidade, proteção) com dominância de ancestral; envelope de reescrita declarado e comparado por fixture; e ordenação estrita de patches que funde inserções no mesmo offset.
  - **28 propriedades verificáveis por máquina**, com as vacuosas substituídas: a de cobertura de envelope deixou de passar com um patch do documento inteiro, a de cardinalidade de slots ganhou uma fixture com três slots nomeados, e a proibição de usar o renderer legado como parser deixou de ser prosa e virou gate de fronteira.

- **Fase 5.0D.3 — Polimento visual, responsividade, mouse e formatação inline.** Revisão integral da experiência apresentada pela TUI real, com navegação por mouse e autoria de cor/marca-texto, sem nenhuma dependência nova e sem alteração no Core.
  - **Identidade e responsividade:** Cabeçalho com o título durável **NOTE-IT — Interface de Terminal**, sem número de fase. Abaixo de 60 colunas apenas o painel focado é exibido; abaixo de 35×6 a interface entra num estado mínimo determinístico. A matriz automatizada cobre 40×12, 80×24 e 140×34, incluindo editor, menu de formatação, Unicode e texto longo.
  - **Rodapé contextual:** Escolha de variantes inteiras por largura, sem cortar um atalho no meio, medida em 40, 50, 60, 70, 80, 100, 120 e 140 colunas. Salvar, Formatar e Sair continuam descobríveis no editor em todas as larguras práticas.
  - **Navegação por mouse com alvos semânticos:** Captura ativa somente durante a tela alternativa e restaurada pelo `TerminalGuard`. Clique nas quatro regiões do cabeçalho abre Notas Recentes, Tarefas Pendentes, Lixeira ou Busca; clique numa linha visível usa a mesma seleção/abertura do teclado; a roda atua no painel sob o ponteiro. Uma troca iniciada por mouse com rascunho pendente passa exatamente pela mesma pergunta salvar/descartar/continuar do teclado. Seleção de texto por mouse permanece fora de escopo.
  - **Formatação inline canônica:** `Alt+F` ou clique no comando do rodapé abre o menu de formatação, navegável por setas/Enter e por clique. Com seleção, a ação é aplicada imediatamente à faixa selecionada; sem seleção, configura o estilo transitório da digitação seguinte, com cor e marca-texto combináveis, trocáveis e limpáveis de forma independente e com indicação visível no título do editor. Alterar apenas esse estado não suja o rascunho nem cria histórico, e cancelar o menu é operação nula.
  - **Paleta compartilhada com a GUI:** Derivada de `ui/src/ui/palettes.ts` — texto Cinza `#64748B`, Vermelho `#DC2626`, Laranja `#C2410C`, Amarelo `#A16207`, Verde `#15803D`, Azul `#2563EB`, Roxo `#7C3AED` e Rosa `#DB2777`; marca-texto Amarelo `#FDE68A` (padrão), Verde `#BBF7D0`, Azul `#BFDBFE`, Rosa `#FBCFE8` e Roxo `#DDD6FE`. A persistência usa exatamente `<span data-note-it-color="…" style="color:…">` e `<mark data-note-it-highlight="…" style="background-color:…">`.
  - **Formatar é mutação do `Draft`:** Preserva Unicode e múltiplas linhas, cria um passo de undo/redo, marca pendência e só alcança o store por `Ctrl+S`, `authority::perform_at` e a `revision` originalmente lida; conflito continua sem sobrescrita. A limpeza remove apenas wrappers canônicos completos dentro da seleção, deixando HTML desconhecido e tags incompletas intactos.
  - **Correção 5.0D.3.R1:** A digitação estilizada agrupa caracteres adjacentes de semântica idêntica em um único wrapper canônico, mantendo cor e marca-texto num aninhamento compatível com a GUI e com o leitor. Avisos transitórios passam a ser limpos na próxima ação significativa, troca de foco/nota/painel, conclusão ou cancelamento da formatação e interação com tarefa.
  - **Tarefas acionáveis:** Em Tarefas Pendentes o texto usa o mesmo renderer inline seguro do leitor, sem vazar delimitadores Markdown ou wrappers canônicos. `Space` alterna a tarefa selecionada e clicar exatamente em `☐`/`☑` executa a mesma ação, enquanto clicar no restante da linha abre a nota; a mutação continua usando o `TaskRef` e a revision carregada pela autoridade transacional do Core.
  - **Descoberta arquitetural 5.0D.3.R2:** Registrada em `docs/tui.md`, seção 15.1 — ocultar markup durante a edição não é polimento seguro sobre o `Draft` atual, porque cursor e seleção são indexados na fonte persistida enquanto o renderer do leitor descarta delimitadores sem mapeamento reversível. Edição visual, mapeamento bidirecional, alternância Visual↔Markdown, matemática e flashcards foram separados para as fases 5.0D.4A, 5.0D.4B e 5.0D.5.

- **Fase 5.0D.2 — Editor nativo no painel direito.** Substituição do `$EDITOR` externo como fluxo principal por edição embutida na própria TUI, sem nenhuma dependência nova e com `Cargo.lock` byte-idêntico.
  - **Abertura sem segunda tecla:** `Enter` sobre uma nota — na lista, na busca ou no painel de tarefas — abre o painel direito já editando. Selecionar com as setas continua pré-visualizando a leitura renderizada da 5.0D.1, e `Esc` desce do editor para a leitura e da leitura para a lista.
  - **Modelo do editor (`noteit-tui/src/draft.rs`):** Corpo em memória indexado por caractere — nunca por byte, que permitiria um cursor dentro de um `ç`, nem por coluna de exibição, que dependeria da fonte. Cobre inserção e remoção Unicode e multilinha, navegação, seleção, quebra de linha, rolagem e desfazer/refazer com limite explícito de 200 passos, agrupando inserções e remoções consecutivas em um único passo.
  - **Salvamento transacional:** `Ctrl+S` grava por `authority::perform_at` com a `revision` lida ao abrir o painel e com `editor::mutation_for`, a mesma função usada pelo `$EDITOR`. Conflito externo nunca sobrescreve conteúdo e oferece preservar o rascunho, reler a nota ou mantê-lo no editor; não há retry automático.
  - **Alterações pendentes nunca somem em silêncio:** Um rascunho pendente só existe com o foco no editor, de modo que trocar de nota/painel, buscar e descartar são inalcançáveis com pendências. `Esc` e o `Ctrl+C` de teclado fazem a mesma pergunta — salvar, descartar conscientemente ou continuar editando — e os sinais externos `SIGINT`, `SIGTERM` e `SIGHUP`, que não têm a quem perguntar, preservam o texto em `tui-recovery` informando o caminho.
  - **Provas:** Os 31 testes de regressão foram escritos primeiro e falharam 29 vezes na baseline `86290a0`. A suíte final tem 45 testes de editor nativo e 144 no crate, com prova em `TestBackend` e em PTY real cobrindo acentuação, colagem multilinha, terminal estreito, redimensionamento, conflito, cancelamento, recuperação e restauração exata do terminal. O `$EDITOR` externo segue disponível na leitura, como alternativa e não como requisito.

- **Fase 5.0D.1 — Fidelidade de Markdown e compatibilidade com notas da GUI.** Correção do leitor antes de ampliar a edição: o renderer passou a consumir com segurança o subconjunto de HTML que o próprio editor gráfico persiste, sem imprimir marcação na tela e sem interpretar HTML arbitrário. Nenhuma dependência nova e `Cargo.lock` byte-idêntico.
  - **Reconhecimento de bloco separado da varredura inline:** `markdown.rs` reconhece blocos linha a linha (fences, comentários, citações, callouts, títulos, separadores, tarefas e listas) e `inline.rs` faz uma varredura segura com pilha de estilos para ênfase, links, código inline, entidades e o subconjunto HTML admitido.
  - **Subconjunto HTML admitido:** `span[data-note-it-color]` e `style=color`, `mark[data-note-it-highlight]` e `style=background-color`, `<u>`, comentários HTML e entidades como `&nbsp;`. Cor de texto e marca-texto compõem entre si e com negrito, itálico e riscado, inclusive dentro de títulos, listas, tarefas, citações e alertas.
  - **Nada vaza e nada é executado:** Nenhum `<span>`, `<mark>`, `<u>`, comentário, entidade ou delimitador reconhecido alcança a tela; conteúdo desconhecido permanece texto seguro e nunca é executado.
  - **Hierarquia H1–H6:** Seis cores próprias mais modificadores, e não apenas H1 a H3, respeitando contraste e terminais de paleta limitada. O painel de tarefas e a prévia da lixeira passaram a usar o mesmo renderer.
  - **Provas:** Os 27 testes de regressão foram escritos primeiro e falharam 24 vezes na baseline `32cbda6`. A suíte final tem 30 testes de fidelidade e 85 no crate, com prova de foreground, background e modificadores em `Span`s e em células do `TestBackend`, além de captura em pseudoterminal real.

- **Fase 5.0D — Modo Edição, Mutação e Concorrência Transacional.** Edição de notas pela TUI com mutação exclusivamente transacional pelo Core, e a revisão corretiva que fechou a fase de verdade.
  - **Mutação sempre revisionada:** Toda escrita passa por `editor::mutation_for` e `authority::perform_at` com a `revision` originalmente lida. Um conflito externo aborta sem sobrescrever um único byte e sem retry automático.
  - **Bloqueio e correção do ciclo de vida do terminal:** O fechamento em `00140b7`, após a implementação `0ce2825` e [CI verde](https://github.com/TheGhols/Note-it/actions/runs/34284207606), foi prematuro: um `$EDITOR` que devolvia o terminal em modo raw contaminava a restauração final. A fase ficou BLOCKED até a revisão `7196a2f`, que fixa T0 como snapshot `termios` imutável tomado antes de qualquer alteração e mantido durante todo o ciclo, acrescenta a regressão em PTY real de falha parcial e passou no [CI corretivo](https://github.com/TheGhols/Note-it/actions/runs/34348671971). O histórico completo — bloqueio, correção e prova — está em `docs/tui.md`, seção 12.

- **Fase 5.0D.R0 — Fechamento do contrato de mutação concorrente.** Contrato fechado antes de abrir a edição na TUI: criação com publicação atômica condicionada à ausência do destino, descarte e restauração condicionados por `revision`, ciclo de descarte compartilhado com o receiver desktop e protocolo privado v3. Gates locais aprovados e [CI da implementação verde](https://github.com/TheGhols/Note-it/actions/runs/34210340965), commit `fc6d39a`. Detalhes e provas em `docs/tui.md`, seção 10.

- **Fase 5.0C — Modo Leitura, Navegação e Apresentação.** Implementação da experiência de navegação por teclado e visualização de notas em Markdown somente-leitura na TUI, operando como cliente in-process de `noteit-core`.
  - **Painéis de navegação por teclado:** Integração de três painéis de navegação lateral: `RecentNotes` (ordenado canonicamente por `updated_at` decrescente via `core.list_summaries`, sem mtime como critério primário), `PendingTasks` (tarefas pendentes extraídas via `core.list_tasks(Pending)` reaproveitando o extrator do Core sem duplicar regex), e `Trash` (listagem de notas descartadas via `core.list_trash()`, exibindo rótulo, data de exclusão e snippet de forma estritamente somente-leitura). Navegação fluida via `Tab` / `BackTab` e teclas numéricas `1`, `2`, `3`.
  - **Busca rápida via tecla `/`:** Ativação instantânea de consulta delegada diretamente a `core.search_notes(&query)`. Reutiliza sem duplicar a normalização de busca, o dobramento de caixa e acentos (`search::fold`), contagem de correspondências e ranqueamento híbrido do Core. Resultados navegáveis por teclado e seleção por `Enter`.
  - **Renderizador de Markdown formatado (`markdown.rs`):** Apresentação formatada no leitor sem mutação: títulos em níveis hierárquicos (H1 a H6), listas com marcadores e numeradas, caixas de seleção visuais (`☐` pendente e `☑` concluída, com remoção automática de comentários de conclusão), citações em bloco e todos os 5 alertas GFM da Fase 3.5 com rótulos visíveis destacados (`[NOTA]`, `[DICA]`, `[IMPORTANTE]`, `[ATENÇÃO]`, `[CUIDADO]`). Blocos de código monoespaçados com linguagem indicada e sem syntax highlighting. Cálculos e expressões matemáticas preservados como texto cru sem avaliação.
  - **Suíte de testes automatizados com `TestBackend` (`tests/navigation_and_rendering.rs`):** 7 novos testes de integração contra stores sintéticos cobrindo recência, atalhos de painéis, extração de tarefas, listagem de lixeira, busca delegada e asserções diretas de buffer em `TestBackend` sem necessidade de display físico. Total de 21 testes na suíte `noteit-tui`.

- **Fase 5.0B — Crate `noteit-tui`, Shell de Terminal e Portão de Fronteira.** Criação do crate dedicado `noteit-tui`, binário executável `target/release/noteit-tui`, shell de terminal com ciclo de vida e restauração segura, e portão mecânico de fronteira.
  - **Crate dedicado `noteit-tui` no workspace:** Configurado em `noteit-tui/` com binário independente. Dependências normais estritas: `ratatui` (0.30.2 com backend `crossterm`), `crossterm` (0.29.0), `signal-hook` (0.3.18) e `noteit-core` (path dependency). Zero dependências de rede (zero HTTP/TLS), zero bibliotecas de interface desktop (zero GTK/WebKit/Wayland) e zero runtime assíncrono (sem `tokio`).
  - **Portão de fronteira mecânico (`scripts/check-tui-boundary`):** Novo gate de qualidade integrado ao `./scripts/check` (`tui-boundary`) e ao CI remoto (`TUI Dependency Boundary`), impedindo de forma contínua a introdução de bibliotecas gráficas de desktop, rede ou desvios de I/O em disco fora do Core.
  - **Guarda RAII e restauração determinística (`TerminalGuard`):** Ativação de modo raw, alternate screen e ocultação de cursor protegidas por destrutor RAII (`Drop`), garantindo de forma infalível a restauração de cooked mode (`disable_raw_mode()`), saída de alternate screen (`LeaveAlternateScreen`), cursor visível (`Show`) e limpeza de buffers na saída normal ou por retorno antecipado.
  - **Hook de pânico blindado (`install_panic_hook`):** Tratador de pânico global que desativa o modo raw e sai da tela alternativa antes de invocar o panic hook padrão do Rust, impedindo que falhas inesperadas deixem o terminal do usuário em estado quebrado ou ilegível.
  - **Tratamento de sinais POSIX (`signal-hook`):** Captura assíncrono-segura de `SIGINT` e `SIGTERM` via flag booleana atômica consultada em loop de eventos síncrono com bounded poll de 50 ms. Saída limpa por `q`, `Esc`, `Ctrl+C` e encerramento por sinal.
  - **Redimensionamento responsivo:** Captura de `Event::Resize` e sincronização via `terminal.autoresize()`, com layout em `ui.rs` desenhado sob restrições dimensionais mínimas para evitar falhas ou estouros em terminais muito pequenos.
  - **Validações de pré-voo fail-fast:** Checagem de existência do store (`paths.notes_dir.exists()`) e validação de terminal interativo (`io::stdout().is_terminal()`) executadas antes de entrar em modo raw. Stores inexistentes ou saídas redirecionadas são rejeitadas com erro explicativo em `stderr` e código 1 sem alterar o estado do terminal.
  - **Testes automatizados com pseudoterminal (`tests/terminal_lifecycle.rs`):** Suíte com 9 testes de integração sobre PTY Unix (`openpty`), comprovando via inspeção de `termios` (`c_lflag`) a ativação de raw mode e sua perfeita restauração após 'q', 'Esc', 'Ctrl+C', sinal 'SIGTERM' e pânico induzido (`--panic-for-test`), além de testar redimensionamento via `TIOCSWINSZ`, rejeição de não-TTY e falha de store ausente.

- **Fase 5.0A — Arquitetura e escopo (TUI + Empacotamento).** Especificação arquitetural, medição de bibliotecas candidatas e definição de escopo para a interface de terminal interativa e distribuição. Nenhum arquivo `.rs` de produção ou teste foi escrito, nenhuma dependência foi adicionada e o `Cargo.lock` permaneceu estritamente byte-idêntico.
  - **`docs/tui.md` (novo):** Documento normativo consolidando as decisões de runtime, fronteira, modelo de concorrência, superfície de dados e plano de entrega.
  - **Runtime medido com números:** Benchmark em ambiente isolado descartável (Rust 1.98 / x86_64, profile release com strip): `crossterm` puro v0.29.0 (32 dependências, 464 KiB stripped, compilação em 7,23 s); `cursive` v0.21.1 (73 dependências, 811 KiB stripped, compilação em 35,56 s); `ratatui` v0.30.2 com backend `crossterm` (85 dependências, 636 KiB stripped, compilação em 20,23 s). Decisão: adoção do **`ratatui`** por seu modelo declarativo imediato (`Terminal::draw`), loop de eventos síncrono limpo sem runtime assíncrono (sem `tokio`), e garantia absoluta de zero dependências de rede e zero dependências gráficas.
  - **Isolamento em crate dedicado (`noteit-tui`):** A TUI residirá no crate `noteit-tui` gerando o binário `target/release/noteit-tui`, e não como subcomando de `noteit-cli`, evitando que o binário leve da CLI receba 85 crates de widgets de tela cheia. O CI receberá o gate `scripts/check-tui-boundary` para certificar ausência de crates de GUI de desktop, ausência de rede e proibição de desvio de storage fora do Core.
  - **Superfície de dados e invariantes:** Vinculação in-process direta ao `noteit-core` (`open_read_only` para consultas e `authority::perform_at` para mutações). Proibição categórica de invocar `noteit`/`note-it` como subprocessos, proibição de parsing de saídas da CLI e de gravação direta de `.md` sem passar pelo Core.
  - **Concorrência transacional com a GUI desktop:** Rejeição da retenção perpétua do lease de escrita (`flock`) pela TUI, que impediria a inicialização da aplicação desktop (`note-it`). A TUI opera como cliente transacional, concorrendo pelo lease apenas no momento da escrita ou delegando ao desktop ativo pelo soquete Unix privado. Concorrência otimista garantida por `expected_revision`, abortando com `RevisionConflict` e zero bytes corrompidos caso a nota seja alterada na GUI enquanto visualizada na TUI.
  - **Auditoria do empacotamento (`packaging/arch/PKGBUILD`):** Identificação de três falhas impeditivas: download 404 para a tag `v0.1.0` inexistente, falha de `makedepends` com `pnpm` não global, e omissão no `package()` de quatro binários essenciais (`noteit`, `noteit-mcp`, `noteit-embed` e `noteit-tui`). Inclusão da modernização do pacote Arch e AUR no escopo da Fase 5.
  - **Escopo delimitado e planejado:** Divisão da Fase 5.0 nas subfases 5.0B (shell e boundary), 5.0C (leitura e navegação), 5.0D (edição e concorrência transacional), 5.0E (empacotamento completo) e 5.0R (auditoria e regressão).
